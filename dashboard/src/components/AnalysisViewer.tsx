...
/* Remove decorative fade overlays and fixed navs generated by LLM */
...
(function(){
  function removeFixedOverlays(){
    document.querySelectorAll('*').forEach(function(el){
      ...
      // Keep if it's a chart legend or tooltip (ECharts uses position:absolute, not fixed)
      var bg=cs.background||'';
      // If it has a gradient background it's a decorative fade — remove it
      if(bg.indexOf('gradient')>=0){ el.style.display='none'; return; }
      // If it has z-index > 50 and is not a chart container — likely a nav/overlay
