...
def get_score_heatmap(days: int = 20) -> dict:
    ...
    返回 {dates: [...], industries: [...], scores: [[...], ...], latest: {行业: 综合分},
          ma: {行业: {ma5, ma20, ma60}}, breakouts: {行业: {日期: [标签]}}}
    ...
    for ind in industries:
        ...
        def _rolling_ma(pos: int, win: int):
            """截至 ma_dates[pos] (含) 的最近 win 个有效值均值, 不足返回 None."""
            idx = [i for i in valid_idx if i <= pos]
            if len(idx) < win:
                return None
