...
A 股 Scout 派监控仪表盘 — 周期定位 / 顶底量化 / 信息情报 / 战略推理 / 9 维度复盘, 整合多源财经数据 + 本地量价 + LLM 推理, 给个人投资者一个"盯盘 + 复盘 + 决策"一体化工作台。

> **Scout 派** = 年轻 + 主动探查。区别于传统的"信息聚合仪表盘", 我们不只展示数据, 还做"主动侦察 + 智能分析"。
...
### 1. 信息聚合 (Scout)
- **财经快讯**: 财联社电报 (含红电报) / 金十数据 / 格隆汇 / 东方财富 / 同花顺 / 第一财经 / 华尔街见闻 — 7 路来源
- **政策动态**: 发改委 / 证监会 / 上交所 / 深交所 / 财新 / 巨潮公告 — 多源 RSS + 结构化抓取
- **市场实时**: 涨停/跌停/炸板/强势股池 / 行业/概念资金流 / 板块排行 / 龙虎榜 / 大单异动 / 人气飙升 / 北向资金 / 雪球热度 / 融资融券 / 大宗交易 / 解禁减持日历
- **研究报告**: 东方财富研报中心今日研报, PDF 在线预览, 按个股/行业/宏观/策略分类

### 2. 周期定位 (Cycle)
- **顶底量化算法** (自管): 14 类技术指标 (PE/股债溢价/动量/换手率/拥挤度/MACD 背离/市场宽度/MA250 偏离等) + 权重优化器 + 周期阶段判定
- **周期信号**: 牛/熊/震荡/底部反转 阶段识别 + 操作建议
- **可视化**: 周期定位图 + 周期分数仪表盘
- **企微推送**: 真发/沙盒双模式, 可选, 关键信号自动推送到群

### 3. QMT 桥接 (Quant)
- **miniQMT 数据 API**: 通过 Windows VM 桥接服务暴露, 拉取实时行情 + 历史数据
- **熔断器 (Breaker)**: QMT 服务异常自动降级, 调用方零改动
- **Dashboard 页面**: QmtDataPage 展示实时 / 历史数据 + 监控状态
- **桥接服务**: `tools/qmt-bridge/` 独立 WSGI, HTTP 协议

### 4. 智能分析 (LLM Scout)
- **Agent 体系**: 盘前/盘中/盘后 4 个时段自动跑, 包含 5 个专项分析师 (新闻/政策/公告/研报/情绪)
- **首席裁决 (chief)**: 综合分析师 + 侦察师 + 情绪 + 风险, 输出最终判断
- **关注股池 (watchlist)**: 单只股票体检 + 整体汇总

### 5. 信息情报简报 (info_brief)
- **4 路信息源综合整理**: 财经快讯 / 政策动态 / 公告 (实质 vs 流程性) / 研报观点
- **LLM 分析输出**: 主旋律 (按重要性) + 跨路关联 (多源共振) + 关注点 (后续跟踪)
- **浅色主题 + 序号渐变 badge + 跨路卡片** — 一眼看出主线
- **时效 badge**: 今天 / 昨天 / N 天前
- **4 cron/天**: morning (6:05) / intraday (10:05, 13:35) / evening (21:05)

### 6. 战略推理 (strategist) — 双保险
- **麦肯锡 6 框架**: PEST / 五力 / 逻辑树 / 归纳演绎 / 决策矩阵 / 金字塔
- **双保险模式**: 读 info_brief 提炼的 3 段 + 独立加载 4 路原始数据做交叉验证
- **9 段固定输出**: 核心判断 + 数据校验 + PEST + 五力 + 逻辑树 + 归纳演绎 + 决策矩阵 + 综合判断 + 跟踪检查点
- **自动联接 chief → info_brief → strategist** (每天 4 次)

### 7. 9 维度复盘 (review)
- **借鉴 DM-kun 7 层框架**, 剔除因子 ICIR, 整合 14 个本地 daily_compute + 板块效应
- **9 维度**: L0 情绪周期 / L0.5 涨停生态 / L1 指数宽度 / L2 风格 / L3 行业 / L3.5 概念 / L4 板块效应 / L5 筹码竞价 / L6 个股 / L7 研报
- **规则化综合判断** (不调 LLM, 纯确定性)
- **盘后 16:00 自动跑**, 落 `review_v2_daily` 表 (跟旧 review_daily 独立)
- **双数据源共存**: 旧 review_daily (板块效应 12 字段) + 新 review_v2_daily (9 维度)
...
| 层级 | 技术 |
|------|------|
| 前端 | React 18 + TypeScript + Vite + Tailwind CSS + Plotly |
...
| LLM 集成 | claude CLI (subprocess, 调 mra-* skills) + 智堡 API + 双保险交叉验证 |
| 存储 | SQLite (`db/market.db`) |
| 本地量价 | stock-trading-data-pro CSV (5000+ 股票 × 250 天) |
| QMT 桥接 | miniQMT (Windows VM) + `tools/qmt-bridge/` HTTP 桥接 |
| 企微推送 | 企业微信机器人 webhook (opt-in 真发 / 默认沙盒) |
